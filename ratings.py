...
def PLUS_rating(info_recv, colour, is_taki, need_to_plus2):
    ...
    if pile_colour == colour or (info_recv['pile']['value'] == '+' and not is_taki):
        # amount of pluses
...
def CHDIR_rating(info_recv, colour, is_taki, need_to_plus2):
    ...
    if pile_colour == colour or info_recv['pile']['value'] == "CHDIR":
        ...
        return max(-0.2, base_ratings["CHDIR"] * (
                    1 - others[(my_place + direction) % len(others)] / others[(my_place - direction) % len(others)]))
    else:
        return -1
